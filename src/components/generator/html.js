...
const layouts = {
  colFormItem(scheme) {
    ...
    const tagDom = tags[config.tag] ? tags[config.tag](scheme) : ''
    // const tagDom = tags[config.tag] ? tags[config.tag](scheme) : 'null'
    // 标题修改---start
    // if (tagDom.length === 0) {
    //   // labelWidth = `label-width="100%"`
    //   // labelWidth = 'label-width="100%!important"'
    //   return `<el-form-item ${labelWidth} prop="${scheme.__vModel__}" ${required}>
    //     ${config.label}
    //   </el-form-item>`
    // }
    // 标题修改---end
  ...
  rowFormItem(scheme) {
    // console.log('schemeRow:', scheme)
    const config = scheme.__config__
    const type = scheme.type === 'default' ? '' : `type="${scheme.type}"`
    const justify = scheme.type === 'default' ? '' : `justify="${scheme.justify}"`
    const align = scheme.type === 'default' ? '' : `align="${scheme.align}"`
    const gutter = scheme.gutter ? `:gutter="${scheme.gutter}"` : 'null'
...
const tags = {
  ...
  'el-divider': el => {
    ...
    const direction = el.type ? `direction="${el.direction}"` : ''
  ...
  'el-tabs': el => {
    ...
    if (child) child = `\n${child}\n` // 换行
    return `<${tag} ${vModel} ${type} ${min} ${max} ${size} ${disabled}>${child}</${tag}>`
  }
}
...
function buildElTabPaneChild(scheme) {
  ...
  const config = scheme.__config__
  if (slot && slot.options && slot.options.length) {
    const tag = config.optionType === 'tabs' ? 'el-tab-pane' : 'el-tab-pane'
    const border = config.border ? 'border' : ''
    children.push(`<${tag} v-for="(item, index) in ${scheme.__vModel__}Options" :key="index" :label="item.value" :disabled="item.disabled" ${border}>{{item.label}}</${tag}>`)
  }
  return children.join('\n')
